...
import { formatRAGPrompt, loadCurrentAuthorityRules } from './rag'
...
export async function handleRecallClick(
  ...
  if (query.includes('CURRENT AUTHORITY') && query.includes('RETRIEVED HISTORICAL EVIDENCE')) {
  ...
  try {
    ...
    showRecallResultsPanel({
      ...
      onConfirm: (selectedResults) => {
        void (async () => {
          const authorityRules = await loadCurrentAuthorityRules()
          opts.injectText(formatRAGPrompt(query, selectedResults, authorityRules))
