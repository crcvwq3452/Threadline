...
function slim(r){
  return {
    ...
    similarityScore:r.similarityScore,metadata:r.metadata
  }
}
...
function parseTemporalWindow(query,referenceIso){
  ...
  const explicit=lower.match(/\b(january|february|march|april|may|june|july|august|september|october|november|december)\s+(20\d{2})\b/)
  if(explicit) return monthWindow(Number(explicit[2]),MONTHS[explicit[1]])

  const ago=lower.match(/\b(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|\d+)\s+months?\s+ago\b/)
  if(ago){
    const n=NUMBER_WORDS[ago[1]] ?? Number(ago[1])
    const ref=new Date(referenceIso)
    // "N months ago" means the calendar month N months before the query month.
    return monthWindow(ref.getUTCFullYear(),ref.getUTCMonth()-n)
  }

  if(/\blast\s+spring\b/.test(lower)){
    const ref=new Date(referenceIso)
    // Benchmark/product default: Northern-hemisphere meteorological spring.
    // "last spring" = the most recent completed Mar-May season.
    let year=ref.getUTCFullYear()
    const springEnd=Date.UTC(year,5,1)
    if(ref.getTime()<springEnd) year-=1
    return {
...
function decomposeQuery(query){
  ...
  const parts=cleaned
    .split(/\b(?:from|through|then|finally|ultimately|after|before|to)\b|[,;:]/i)
    .map(s=>s.replace(/^(?:and|but|so)\s+/i,'').trim())
    .filter(s=>s.length>=8)

  // Add compact clause tails because verbose question framing ("How did ...")
  // can hurt exact/lexical retrieval. Keep this mechanical and observable.
  const tails=[]
  for(const p of parts){
    const m=p.match(/\b(initial|original|early|provenance|problem|correction|final|current|later|policy|rule|decision)\b.*$/i)
    if(m && m[0].length>=6) tails.push(m[0].trim())
  }
  return [...new Set([...parts,...tails])].filter(s=>s.toLowerCase()!==cleaned.toLowerCase()).slice(0,8)
}
const EXPANSION_STOP=new Set([
  'Project','Transport','Initial','Final','Historical','Later','Current','Architecture',
  'The','This','That','These','Those','Early','Later','Policy','Rule','Source','Threadline',
  'ChatGPT','Synthetic','Retrieval','Evidence','Governance','Orbital','Quartz'
])
function expansionTerms(query,rows){
  const qlow=query.toLowerCase()
  const terms=[]
  for(const row of rows.slice(0,3)){
    const text=row.content||''
    const matches=text.match(/\b(?:[A-Z][a-z]{3,}|[A-Z][A-Z0-9_]{3,})\b/g)||[]
    for(const term of matches){
      if(EXPANSION_STOP.has(term)) continue
      if(qlow.includes(term.toLowerCase())) continue
...
async function searchRows(ext,query,topK=30){
  const response=await runtimeMessage(ext,{type:'SEARCH_MEMORIES',payload:{query,topK}})
  return (response?.payload?.results||[]).map(slim)
}
async function orchestratedSearch(ext,q,referenceIso){
  const temporal=parseTemporalWindow(q.query,referenceIso)
  const initial=await searchRows(ext,q.query,temporal?100:40)
  const eligibleInitial=q.exclude_session?initial.filter(r=>r.sessionId!==q.exclude_session):initial

  if(temporal){
    const inside=eligibleInitial.filter(r=>{
      const ms=rowMillis(r)
      return Number.isFinite(ms) && ms>=temporal.start && ms<temporal.end
    })
    return {
      rows:uniqueRows(inside).slice(0,10),
      raw:initial,
      strategy:{type:'temporal-filter',window:temporal,overfetch:initial.length}
    }
  }

  const selected=[]
  if(eligibleInitial[0]) selected.push(eligibleInitial[0])

  // Multi-aspect decomposition: give each meaningful clause one chance to
  // contribute evidence before ordinary rank filling.
  const subqueries=decomposeQuery(q.query)
  const subqueryDebug=[]
  for(const sub of subqueries){
    const hits=(await searchRows(ext,sub,12))
      .filter(r=>!q.exclude_session||r.sessionId!==q.exclude_session)
    const candidate=hits.find(r=>!selected.some(s=>s.id===r.id))
    if(candidate) selected.push(candidate)
    subqueryDebug.push({query:sub,top:hits.slice(0,3).map(r=>r.id),picked:candidate?.id??null})
    if(selected.length>=5) break
  }

  // Second-hop expansion: discover a named identifier from top evidence that
  // did not occur in the original query, search it, and add novel evidence.
  const expansionDebug=[]
  const terms=expansionTerms(q.query,eligibleInitial)
  for(const term of terms){
    const hits=(await searchRows(ext,term,12))
      .filter(r=>!q.exclude_session||r.sessionId!==q.exclude_session)
    const candidate=hits.find(r=>!selected.some(s=>s.id===r.id))
    if(candidate) selected.push(candidate)
    expansionDebug.push({term,top:hits.slice(0,3).map(r=>r.id),picked:candidate?.id??null})
    if(selected.length>=5) break
  }

  for(const row of eligibleInitial){
    if(!selected.some(s=>s.id===row.id)) selected.push(row)
    if(selected.length>=10) break
  }
  return {
    rows:uniqueRows(selected).slice(0,10),
    raw:initial,
    strategy:{type:'decompose-expand',subqueries:subqueryDebug,expansions:expansionDebug}
  }
}
...
;(async()=>{
  ...
  const referenceIso='2026-09-22T12:00:00Z'
  const output={
    engine:'threadline-v0.16+transparent-orchestration',
    head:'7f67be4372d56edcd381c90ea8e774de4d8ab360',
    reference_time:referenceIso,
    cases:[]
  }
  for(const q of fixture.queries){
    const got=await orchestratedSearch(ext,q,referenceIso)
    const rows=got.rows
    const raw=got.raw
    const ev=evaluate(q,rows)
    const rawTop=raw.slice(0,5).map(r=>({id:r.id,sessionId:r.sessionId,similarityScore:r.similarityScore}))
    output.cases.push({
      id:q.id,query:q.query,expected:q.expected,expected_all_top5:q.expected_all_top5,
      exclude_session:q.exclude_session,evaluation:ev,strategy:got.strategy,
      raw_top5:rawTop,results:rows
    })
    console.log(JSON.stringify({
      engine:'threadline-orchestrated',case:q.id,pass:ev.pass,rank:ev.rank,
      raw_top:rawTop.map(x=>x.id),orchestrated_top:rows.slice(0,5).map(x=>x.id),
      strategy:got.strategy,reasons:ev.reasons
    }))
  }
  output.summary={
