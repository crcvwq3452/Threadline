...
;(async()=>{
  ...
  const output={engine:'threadline-v0.16-transparent-orchestration',head:'7f67be4372d56edcd381c90ea8e774de4d8ab360',cases:[]}
  for(const q of fixture.queries){
    ...
    // Transparent bounded orchestration for query shapes that explicitly ask for multiple evidence stages.
    if(q.category==='evolution'){
      const clauses=String(q.query)
        .split(/,\s*|\bthrough\b|\bto the\b/i)
        .map(s=>s.replace(/[?]+$/,'').trim())
        .filter(s=>s.length>=6)
      const stageHeads=[]
      for(const clause of clauses.slice(0,4)){
        const sr=await runtimeMessage(ext,{type:'SEARCH_MEMORIES',payload:{query:clause,topK:8}})
        const rr=(sr?.payload?.results||[]).map(slim)
        if(rr[0]) stageHeads.push(rr[0])
      }
      const seen=new Set()
      raw=[...stageHeads,...raw].filter(r=>{
    ...
    // One bounded second hop: expand only with a newly surfaced proper entity from the first hit.
    if(q.category==='multihop' && raw[0]){
      const queryText=String(q.query).toLowerCase()
      const proper=[...String(raw[0].content||'').matchAll(/\b[A-Z][A-Za-z0-9_-]{3,}\b/g)]
        .map(m=>m[0])
        .filter((v,i,a)=>a.indexOf(v)===i)
        .filter(v=>!queryText.includes(v.toLowerCase()))
      if(proper.length){
        const hop=await runtimeMessage(ext,{type:'SEARCH_MEMORIES',payload:{query:proper[0],topK:12}})
        const hopRows=(hop?.payload?.results||[]).map(slim)
        const seen=new Set()
        raw=[raw[0],...hopRows,...raw.slice(1)].filter(r=>{
          if(seen.has(r.id)) return false
          seen.add(r.id)
          return true
        })
